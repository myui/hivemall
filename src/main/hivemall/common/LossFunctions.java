...
public final class LossFunctions {
    ...
    public static abstract class BinaryLoss implements LossFunction {
        ...
        static void checkTarget(final float y) {
            if(!(y == 1.f || y == -1.f)) {
    ...
    public static final class HingeLoss extends BinaryLoss {
        ...
        @Override
        public float loss(float p, float y) {
            float loss = LossFunctions.hingeLoss(p, y, threshold);
    ...
    public static final class EpsilonInsensitiveLoss extends RegressionLoss {
        ...
        @Override
        public float loss(float p, float y) {
            float loss = epsilonInsensitiveLoss(p, y, epsilon);
            return (loss > 0.f) ? loss : 0.f;
        }
     ...
     * @return v - epsilon
     */
    public static float epsilonInsensitiveLoss(final float v, final float epsilon) {
        return v - epsilon;
    }
     ...
     */
    public static float epsilonInsensitiveLoss(final float predicted, final float target, final float epsilon) {
        return Math.abs(target - predicted) - epsilon;
    }
